...
public class F1 extends PApplet {
...
public void setup() {
  ...
  Location location = new Location(38.736946f, -9.142685f);
...
public void page1() {
  ...
  for (Map.Entry raceEntry : racesMap.entrySet()) {
    ...
    ellipse(mapPosition.x, mapPosition.y, 0.75f * scale, 0.75f * scale);
    ...
    if (mouseX < mapPosition.x + 7 && mouseX > mapPosition.x - 7 && mouseY < mapPosition.y + 7 && mouseY > mapPosition.y - 7) {
      ...
      text(circuitJSON.getString("circuitName"), 150, 135);
       ...
       image(video, 50, 150, video.width * 0.75f, video.height * 0.75f);
       video.loop();
  ...
  if (!dataLoaded) {
    ...
    for (int i = 0; i < constructorsJSON.size(); i++) {
      ...
      if (i < constructorsJSON.size() / 2) {
        image(constructorImage, incr1 * 100, canvasHeight * .33f);
        incr1++;

        // When user hovers the constructor image
        if (mouseX < i * 100 + 50 && mouseX > i * 100 -50 && mouseY > canvasHeight * .33f - 50 && mouseY < canvasHeight * .33f +50) {
        ...
        image(constructorImage, incr2 * 100, canvasHeight * .66f);
        incr2++;

        // When user hovers the constructor image
        if (mouseX < i * 100 + 50 && mouseX > i * 100 -50 && mouseY > canvasHeight * .66f - 50 && mouseY < canvasHeight * .66f +50) {
          // If user clicks on the image
          if (mousePressed) {
            selectedConstructor = constructorJSON;
            currentPage = 5;
...
public void page6() {
  if (!dataLoaded) {
    ...
    for (int i = 0; i < finishStatusesJSON.size(); i++) {  
      ...
      int randomColor = color(random(255), random(255), random(255), random(255));
      fill(randomColor); 
...
public void loadCircuitPointOnMap() {
...
class MapPosition {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "F1" };
    if (passedArgs != null) {
